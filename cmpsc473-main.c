...
#include <limits.h>
/* Defines */
#define OBJ_LEN     84   // CHANGE: size of object tree for this project
#define KEY_LEN     8
...
/* Project APIs */
// public 
extern int set_objec( char *objname,  char *filename );
...
int set_objec( char *objname, char *filename )
{
...
struct B *upload_B( FILE *fp )
{
	...
	while(1) {
		...
		if (( ref = strstr( line, "num_a" ))) {
			if (safe_atoi(ref+6, &(objB->num_a))!=0){
				printf("erro objB->num_a\n");}
		}
		if (( ref = strstr( line, "num_b" ))) {
			if (safe_atoi(ref+6, &(objB->num_b))!=0){
				printf("erro objB->num_b\n");}
		}
		if (( ref = strstr( line, "num_c" ))) {
			if (safe_atoi(ref+6, &(objB->num_c))!=0){
				printf("erro objB->num_c\n");}
		}
		if (( ref = strstr( line, "num_d" ))) {
			if (safe_atoi(ref+6, &(objB->num_d))!=0){
				printf("erro objB->num_d\n");}
		}
...
{  
	...
	while(1) {
		...
		line[strlen(line)-1] = '\0';   
		if (( ref = strstr( line, "num_a" ))) {
			if (safe_atoi(ref+6, &(objC->num_a))!=0){
				printf("erro objC->num_a\n");}
		}
		if (( ref = strstr( line, "num_b" ))) {
			if (safe_atoi(ref+6, &(objC->num_b))!=0){
				printf("erro objC->num_b\n");}
		}
		if (( ref = strstr( line, "num_c" ))) {
			if (safe_atoi(ref+6, &(objC->num_c))!=0){
				printf("erro objC->num_c\n");}
		}
	}
	return objC;
}
...
unsigned char *marshall( struct A *objA )
{
	...
	memcpy(obj+STRLEN,&(objA->string_b),STRLEN);
	memcpy(obj+(2*STRLEN),&(objA->ptr_c->num_a),sizeof(int));
	memcpy(obj+(2*STRLEN)+(sizeof(int)),&(objA->ptr_c->num_b),sizeof(int));
	memcpy(obj+(2*STRLEN)+(2*sizeof(int)),&(objA->ptr_c->num_c),sizeof(int));
	memcpy(obj+(2*STRLEN)+(3*sizeof(int)),&(objA->ptr_c->num_d),sizeof(int));
	memcpy(obj+(2*STRLEN)+(4*sizeof(int)),&(objA->ptr_c->string_e),STRLEN);
	memcpy(obj+(3*STRLEN)+(4*sizeof(int)),&(objA->ptr_d->num_a),sizeof(int));
	memcpy(obj+(3*STRLEN)+(5*sizeof(int)),&(objA->ptr_d->num_b),sizeof(int));
	memcpy(obj+(3*STRLEN)+(6*sizeof(int)),&(objA->ptr_d->num_c),sizeof(int));
...
struct A *unmarshall( unsigned char *obj )
{
	...
	memcpy(&(objA->string_b),obj+STRLEN,STRLEN);
	memcpy(&(objA->ptr_c->num_a),obj+(2*STRLEN),sizeof(int));
	memcpy(&(objA->ptr_c->num_b),obj+(2*STRLEN)+(sizeof(int)),sizeof(int));
	memcpy(&(objA->ptr_c->num_c),obj+(2*STRLEN)+(2*sizeof(int)),sizeof(int));
	memcpy(&(objA->ptr_c->num_d),obj+(2*STRLEN)+(3*sizeof(int)),sizeof(int));
	memcpy(&(objA->ptr_c->string_e),obj+(2*STRLEN)+(4*sizeof(int)),STRLEN);
	memcpy(&(objA->ptr_d->num_a),obj+(3*STRLEN)+(4*sizeof(int)),sizeof(int));
	memcpy(&(objA->ptr_d->num_b),obj+(3*STRLEN)+(5*sizeof(int)),sizeof(int));
	memcpy(&(objA->ptr_d->num_c),obj+(3*STRLEN)+(6*sizeof(int)),sizeof(int));
	////
